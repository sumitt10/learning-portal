		...
		try {
			...
			if (userEntity.isPresent() && courseEntity.isPresent()) {
				...
				if (enrollmentService.checkEnrollmentByUserAndCourse(user, course)) {
					...
					List<EnrollmentEntity> registrations = enrollmentService.findEnrollmentByUserEntity(user);
					Optional<EnrollmentEntity> registrationOpt = registrations.stream()
							.filter(reg -> reg.getCourseEntity() == null).findFirst();
					EnrollmentEntity registrationEntity;
					if (registrationOpt.isEmpty()) {
						registrationEntity = new EnrollmentEntity();
						registrationEntity.setUserEntity(user);
					} else {
						registrationEntity = registrationOpt.get();
					}
					registrationEntity.setCourseEntity(course);
					enrollmentService.saveEnrollment(registrationEntity);
					...
					EnrollmentEntity registrationEntity = new EnrollmentEntity();
					registrationEntity.setUserEntity(user);
					registrationEntity.setCourseEntity(course);
					enrollmentService.saveEnrollment(registrationEntity);
